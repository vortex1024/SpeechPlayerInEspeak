...
class SynthDriver(SynthDriver):
	...
	description = "speech player in espeak"
	...
	def __init__(self):
		_espeak_sp.initialize(self._onIndexReached)
		log.info("Using eSpeak NG version %s" % _espeak_sp.info())
		lang=languageHandler.getLanguage()
		_espeak_sp.setVoiceByLanguage(lang)
		self._language=lang
		self._variantDict=_espeak_sp.getVariantDict()
		self.variant="max"
		self.rate=30
		self.pitch=40
		self.inflection=75
	...
	def speak(self,speechSequence):
		...
		for item in speechSequence:
			if isinstance(item,str):
				...
				try:
					phonemes="".join([self.IPA_TO_espeak_sp[char] for char in item.ipa])
		...
		_espeak_sp.speak(text)

	def cancel(self):
		_espeak_sp.stop()

	def pause(self,switch):
		_espeak_sp.pause(switch)
	...
	def _get_rate(self):
		val=_espeak_sp.getParameter(_espeak_sp.espeakRATE,1)
		if self._rateBoost:
			val=int(val/self.RATE_BOOST_MULTIPLIER)
		return self._paramToPercent(val,_espeak_sp.minRate,_espeak_sp.maxRate)

	def _set_rate(self,rate):
		val=self._percentToParam(rate, _espeak_sp.minRate, _espeak_sp.maxRate)
		if self._rateBoost:
			val=int(val*self.RATE_BOOST_MULTIPLIER)
		_espeak_sp.setParameter(_espeak_sp.espeakRATE,val,0)

	def _get_pitch(self):
		val=_espeak_sp.getParameter(_espeak_sp.espeakPITCH,1)
		return self._paramToPercent(val,_espeak_sp.minPitch,_espeak_sp.maxPitch)

	def _set_pitch(self,pitch):
		val=self._percentToParam(pitch, _espeak_sp.minPitch, _espeak_sp.maxPitch)
		_espeak_sp.setParameter(_espeak_sp.espeakPITCH,val,0)

	def _get_inflection(self):
		val=_espeak_sp.getParameter(_espeak_sp.espeakRANGE,1)
		return self._paramToPercent(val,_espeak_sp.minPitch,_espeak_sp.maxPitch)

	def _set_inflection(self,val):
		val=self._percentToParam(val, _espeak_sp.minPitch, _espeak_sp.maxPitch)
		_espeak_sp.setParameter(_espeak_sp.espeakRANGE,val,0)

	def _get_volume(self):
		return _espeak_sp.getParameter(_espeak_sp.espeakVOLUME,1)

	def _set_volume(self,volume):
		_espeak_sp.setParameter(_espeak_sp.espeakVOLUME,volume,0)

	def _getAvailableVoices(self):
		voices=OrderedDict()
		for v in _espeak_sp.getVoiceList():
			l=_espeak_sp.decodeEspeakString(v.languages[1:])
			# #7167: Some languages names contain unicode characters EG: Norwegian Bokmål
			name=_espeak_sp.decodeEspeakString(v.name)
			# #5783: For backwards compatibility, voice identifies should always be lowercase
			identifier=os.path.basename(_espeak_sp.decodeEspeakString(v.identifier)).lower()
	...
	def _get_voice(self):
		...
		curVoice = _espeak_sp.getCurrentVoice()
		if not curVoice:
			return ""
		# #5783: For backwards compatibility, voice identifies should always be lowercase
		return _espeak_sp.decodeEspeakString(curVoice.identifier).split('+')[0].lower()
	...
	def _set_voice(self, identifier):
		...
		try:
			_espeak_sp.setVoiceAndVariant(voice=identifier,variant=self._variant)
		except:
			self._voice=None
			raise
	...
	def _set_variant(self,val):
		...
		_espeak_sp.setVoiceAndVariant(variant=self._variant)
